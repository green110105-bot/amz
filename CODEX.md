...
## 🚫 第 0 铁律（最高优先级）：新功能一律接真实数据源，绝对不用 mock 数据充数

> 用户明令：**绝对不可以再用 mock 数据。** 适用边界 = **今后任何新开发 / 新接数据，必须接真实数据源，不准用 mock / 伪造 / 随机数 / 写死示例 来"填充看板、糊一个能跑的样子"。**

**正面范例（必须照此做）**：TikTok 日报看板——宁可去逆向 lobster 服务器、Node 复刻领星 AES 签名、接通真实领星 OpenAPI 并对真实 API 验证（拿到真实销量），也**不**糊一个假看板。

**硬性要求**：
1. 新功能要展示数据 → 先找到并接通**真实数据源**（真实 API / 真实 DB / 真实凭证）。找不到源 = 这个功能**先不做**，而不是用 mock 顶上。
2. 真实源需要凭证/IP 白名单/网络 → 当作**前置阻塞**列出来交给用户解决，不准用 mock 绕过假装做完了。
3. 真实数据暂时拉不到（凭证未配 / 拉取失败）→ 唯一允许的兜底是**明确标注 `mock:true` + 失败原因 + 空态**，并且**必须同时实现真实通路**（凭证一到就自动切真）。绝不允许"只做 mock 不做真实通路"。
4. **禁止**：为了让新看板/新页面"看起来有数据"而用 `Math.random` / 写死数组 / mulberry32 / 假事件 造数。这类一经发现即判定未完成。

**边界澄清（不炸现有系统）**：
- 本铁律针对**新开发**。现有 demo 的 34 个前端 mock-data、66 个测试 mock、provider-mode 的 mock/hybrid 分支**不一刀切删除**（删了会炸 1048 测试 + demo 登录）。它们按模块逐步替换为真实源，替换前保持现状但**不得新增**对它们的依赖。
- 测试里用确定性 fixture/已知答案锚点（如领星签名测试）**不算违规**——那是"判据"，不是"假数据充数"。
- 受真实 Amazon 凭证缺失硬约束、且**已诚实标注 + 已实现真实通路预留**的项（M3 真实写、M1 发布等）维持现状，凭证到位即接真。

> 一句话：**能接真就必须接真；接不了真就先别做，并把阻塞交出去——绝不用 mock 假装做完了。**
